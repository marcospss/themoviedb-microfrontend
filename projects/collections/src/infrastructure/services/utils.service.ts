...
export class UtilsService {
  ...
  posterImage(poster_path: string, size:string): string {
    const sizeImage = environment.imageApi.poster_sizes.w92;
    if (!poster_path) {
  ...
  backdropImage(backdrop_path: string, size: string = 'w300'): string {
    const sizeImage = environment.imageApi.backdrop_sizes.w300;
    if (!backdrop_path) {
      return 'assets/images/default-image.png';
    }
